<div class="dropdown dropdown-end">
    <div tabindex="0" role="button" class="btn btn-ghost btn-sm px-2 gap-1.5 flex items-center" title="{{ __('Pilih Bahasa') }}">
    ...
    <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box shadow-lg border border-base-300 w-36 mt-2 p-1.5 z-50">
        <li>
            <a href="{{ route('language.switch', 'id') }}" class="flex items-center justify-between py-2 text-xs {{ app()->getLocale() === 'id' ? 'active font-semibold' : '' }}">
                <span>🇮🇩 Indonesia</span>
                @if(app()->getLocale() === 'id')
        ...
        <li>
            <a href="{{ route('language.switch', 'en') }}" class="flex items-center justify-between py-2 text-xs {{ app()->getLocale() === 'en' ? 'active font-semibold' : '' }}">
                <span>🇬🇧 English</span>
                @if(app()->getLocale() === 'en')
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                @endif
