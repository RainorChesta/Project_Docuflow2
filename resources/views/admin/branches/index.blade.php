                        ...
                        <tbody>
                            @forelse($branches as $branch)
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <a href="{{ route('admin.branches.edit', $branch) }}" class="btn btn-ghost btn-xs btn-square text-primary" title="{{ __('Edit') }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        </a>
                                        @if(!$branch->is_pusat)
                                            <form method="POST" action="{{ route('admin.branches.destroy', $branch) }}" class="inline ml-2" onsubmit="return confirm('{{ __('Hapus cabang ini?') }}')">
                                                @csrf @method('DELETE')
