            ...
            <div class="card bg-base-100 border border-base-300 shadow-sm p-4 sm:p-6">
                <form method="POST" action="{{ route('admin.branches.update', $branch) }}">
                    ...
                    @if(!$branch->is_pusat)
                        ...
                        {{-- PIC Klinik / Penanggung Jawab Cabang (Hanya untuk Cabang, bukan Pusat) --}}
                        <div class="form-control w-full mb-5">
                            <label for="pic_klinik_id" class="label">
                                <span class="label-text font-medium">{{ __('PIC Klinik / Kepala Cabang') }}</span>
                                <span class="label-text-alt text-base-content/60">{{ __('Penanggung Jawab Dokumen Cabang') }}</span>
                            </label>
                            <div x-data="{
                                search: '',
                                open: false,
                                value: '{{ old('pic_klinik_id', $branch->pic_klinik_id ?? '') }}',
                                options: [
                                ...
                                <input type="hidden" name="pic_klinik_id" x-model="value">
