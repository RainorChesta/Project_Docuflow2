    ...
    <div class="py-6" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto w-full space-y-4">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                @if($trashedDocuments->total() > 0)
                    <form method="POST" action="{{ route('admin.trash.empty') }}" onsubmit="return confirm('{{ __('Apakah Anda yakin ingin menghapus PERMANEN SELURUH dokumen di tempat sampah? Tindakan ini tidak dapat dibatalkan!') }}')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-error btn-sm gap-2 shadow-sm text-white">
            ...
            <div x-show="selected.length > 0" 
                ...
                <div class="flex items-center gap-2">
                    {{-- Bulk Restore Form --}}
                    <form method="POST" action="{{ route('admin.trash.bulk-restore') }}" class="inline" onsubmit="return confirm('{{ __('Pulihkan dokumen yang dipilih?') }}')">
                        @csrf
                        <template x-for="id in selected" :key="'restore-'+id">
                            <input type="hidden" name="ids[]" :value="id">
                        </template>
                        <button type="submit" class="btn btn-sm bg-white text-primary hover:bg-white/90 border-0 gap-1 font-semibold">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <form method="POST" action="{{ route('admin.trash.bulk-force-delete') }}" class="inline" onsubmit="return confirm('{{ __('Apakah Anda yakin ingin menghapus PERMANEN dokumen yang dipilih? Tindakan ini tidak dapat dibatalkan!') }}')">
                        @csrf
                        @method('DELETE')
                        <template x-for="id in selected" :key="'delete-'+id">
                            <input type="hidden" name="ids[]" :value="id">
                        </template>
                        <button type="submit" class="btn btn-sm btn-error text-white gap-1 font-semibold">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            </svg>
            ...
            <div class="card bg-base-100 border border-base-300 shadow-sm p-4">
                <form method="GET" action="{{ route('admin.trash.index') }}" class="flex items-center gap-3">
                    <div class="relative flex-1">
                        ...
                        @if($search)
                            <a href="{{ route('admin.trash.index') }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-base-content/40 hover:text-base-content">
                                ...
                                <tr class="hover:bg-base-200/30 transition-colors" :class="selected.includes({{ $doc->id }}) ? 'bg-primary/5' : ''">
                                    ...
                                    <td class="text-right">
                                        <div class="flex items-center justify-end gap-1">
                                            {{-- Restore Form --}}
                                            <form method="POST" action="{{ route('admin.trash.restore', $doc->id) }}" class="inline" onsubmit="return confirm('{{ __('Pulihkan dokumen ":title"?', ['title' => $doc->title]) }}')">
                                                @csrf
                                                <button type="submit" class="btn btn-ghost btn-xs btn-square text-success hover:bg-success/10" title="{{ __('Pulihkan Dokumen') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                                                    </svg>
                                                </button>
                                            ...
                                            {{-- Force Delete Form --}}
                                            <form method="POST" action="{{ route('admin.trash.force-delete', $doc->id) }}" class="inline" onsubmit="return confirm('{{ __('Hapus PERMANEN dokumen ":title"? Action ini tidak dapat dibatalkan!', ['title' => $doc->title]) }}')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-ghost btn-xs btn-square text-error hover:bg-error/10" title="{{ __('Hapus Permanen') }}">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                    </svg>
                                                </button>
