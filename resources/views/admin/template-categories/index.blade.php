                        ...
                        <tbody>
                            @forelse($categories as $category)
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <a href="{{ route('admin.template-categories.edit', $category) }}" class="btn btn-ghost btn-xs btn-square text-primary" title="{{ __('Edit') }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        </a>
                                        <form method="POST" action="{{ route('admin.template-categories.destroy', $category) }}" class="inline" onsubmit="return confirm('{{ __('Hapus kategori ini?') }}')">
                                            @csrf @method('DELETE')
                                            <button class="btn btn-ghost btn-xs btn-square text-error ml-1" title="{{ __('Hapus') }}">
                    ...
                    @if($categories->hasPages())
                        <div class="p-4 border-t border-base-200 flex flex-col sm:flex-row items-center justify-between gap-3">
                            <p class="text-sm text-base-content/60 shrink-0">
                                {{ __('Menampilkan :first sampai :last dari :total data', ['first' => $categories->firstItem(), 'last' => $categories->lastItem(), 'total' => $categories->total()]) }}
                            </p>
                            <div class="shrink-0">
