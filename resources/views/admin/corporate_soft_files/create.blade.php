            ...
            <div class="card bg-base-100 border border-base-300 shadow-sm rounded-2xl overflow-hidden">
                <div class="card-body p-6">
                    ...
                    <form method="POST" action="{{ route('admin.corporate-soft-files.store') }}" enctype="multipart/form-data" class="space-y-6">
                                    ...
                                    <div class="flex flex-col items-center justify-center space-y-2 py-2">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-base-content/50 mt-1">
                                                {{ __('Mendukung: Word (.docx, .doc), Dokumen PDF (.pdf) — Maks 15 MB') }}
                                            </p>
                                        </div>
                                    </div>
                                </template>
                        ...
                        {{-- Company Access --}}
                        <div x-data="{ allCompanies: {{ old('is_all_companies', '1') == '1' ? 'true' : 'false' }} }" class="space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                <label class="label cursor-pointer gap-2 p-0">
                                    ...
                                    <input type="checkbox" name="is_all_companies" value="1" x-model="allCompanies" {{ old('is_all_companies', '1') == '1' ? 'checked' : '' }} class="toggle toggle-primary toggle-sm">
                                </label>
                            </div>

                            <div x-show="!allCompanies" x-cloak x-transition class="p-4 bg-base-200/50 rounded-2xl border border-base-300 space-y-2" style="{{ old('is_all_companies', '1') == '1' ? 'display: none;' : '' }}">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                                    @foreach($companies as $company)
                                        <label class="label cursor-pointer justify-start gap-2.5 p-1 hover:bg-base-300/40 rounded-lg">
                                            <input type="checkbox" name="company_ids[]" value="{{ $company->id }}" class="checkbox checkbox-primary checkbox-sm rounded-md" {{ is_array(old('company_ids')) && in_array($company->id, old('company_ids')) ? 'checked' : '' }}>
                        ...
                        {{-- Branch Access --}}
                        <div x-data="{ allBranches: {{ old('is_all_branches', '1') == '1' ? 'true' : 'false' }} }" class="space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                <label class="label cursor-pointer gap-2 p-0">
                                    ...
                                    <input type="checkbox" name="is_all_branches" value="1" x-model="allBranches" {{ old('is_all_branches', '1') == '1' ? 'checked' : '' }} class="toggle toggle-secondary toggle-sm">
                                </label>
                            </div>

                            <div x-show="!allBranches" x-cloak x-transition class="p-4 bg-base-200/50 rounded-2xl border border-base-300 space-y-2" style="{{ old('is_all_branches', '1') == '1' ? 'display: none;' : '' }}">
                                <span class="text-xs text-base-content/60 font-medium block mb-2">{{ __('Pilih Cabang yang dapat mengakses:') }}</span>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                                    @foreach($branches as $branch)
                                        <label class="label cursor-pointer justify-start gap-2.5 p-1 hover:bg-base-300/40 rounded-lg">
                                            <input type="checkbox" name="branch_ids[]" value="{{ $branch->id }}" class="checkbox checkbox-secondary checkbox-sm rounded-md" {{ is_array(old('branch_ids')) && in_array($branch->id, old('branch_ids')) ? 'checked' : '' }}>
