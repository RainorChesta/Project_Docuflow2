<form method="GET" action="{{ route('documents.index') }}" class="mb-4 flex flex-col sm:flex-row flex-wrap gap-2 items-stretch sm:items-center w-full">
    ...
    @if(isset($unitKerjas))
    <select name="unit_kerja_id" class="select select-bordered select-sm w-full sm:w-auto" onchange="this.form.submit()">
        <option value="">{{ __('Semua Unit Kerja') }}</option>
        @foreach($unitKerjas as $uk)
            <option value="{{ $uk->id }}" {{ request('unit_kerja_id') == $uk->id ? 'selected' : '' }}>
                {{ $uk->nama_unit_kerja }} ({{ strtoupper($uk->kode_unit_kerja) }})
            </option>
        @endforeach
    </select>
    @endif
    ...
    <div class="flex flex-wrap gap-2 items-center sm:ml-auto shrink-0 w-full sm:w-auto justify-between sm:justify-end mt-1 sm:mt-0">
        <div class="flex items-center gap-2">
            ...
            @if(request('search') || request('status') || request('unit_kerja_id') || request('document_type_id') || request('filter_branch_id') || request('year') || request('format_choice'))
