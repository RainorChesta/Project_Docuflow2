<x-app-layout>
    <x-slot name="header">{{ __('Dokumen Unit Kerja') }}</x-slot>
    ...
    <div class="pt-0 pb-6">
        <div class="max-w-7xl mx-auto w-full" x-data="{ viewMode: localStorage.getItem('docViewMode') || 'list' }" x-init="$watch('viewMode', val => localStorage.setItem('docViewMode', val))">
            @include('documents._header', ['title' => __('Dokumen Unit Kerja'), 'showCreate' => false])
            ...
            <div x-show="viewMode === 'list'" class="card bg-base-100 border border-base-300 shadow-sm mb-4">
                <div class="card-body p-0">
                    <div class="divide-y divide-base-200">
                        ...
                        @empty
                            <div class="p-4 sm:p-6 text-base-content/60">{{ __('Tidak ada dokumen unit kerja.') }}</div>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div x-show="viewMode === 'grid'" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 sm:gap-3 mb-4 bg-base-100 border border-base-300 rounded-xl shadow-sm p-3" style="display: none;">
                ...
                @empty
                    <div class="col-span-full p-4 sm:p-6 text-base-content/60 bg-base-100 rounded-xl border border-base-300 shadow-sm">
                        {{ __('Tidak ada dokumen unit kerja.') }}
                    </div>
                @endforelse
            </div>
