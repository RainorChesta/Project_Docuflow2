                        ...
                        @can('approve', $document)
                            <div class="flex flex-wrap gap-2 shrink-0">
                                <form method="POST" action="{{ route('approvals.rollback-request.approve', $document) }}" class="inline">
                                    @csrf
                                    <button class="btn btn-success btn-sm" onclick="return confirm('{{ __('Yakin?') }} {{ __('Versi setelah') }} v{{ $document->pendingRollbackVersion->version_number }} {{ __('akan dihapus permanen dan tidak bisa dikembalikan.') }}')">{{ __('Approve') }} Rollback</button>
                                </form>
                                <form method="POST" action="{{ route('approvals.rollback-request.reject', $document) }}" class="inline">
                                    @csrf
                                    <button class="btn btn-error btn-sm">{{ __('Reject') }}</button>
                                </form>
                            </div>
                        @endcan
                    </div>
