                            ...
                            @if($pendingSigRequest)
                                ...
                                @php
                                    $hasPendingSignature = \App\Models\SignatureRequest::where('document_id', $document->id)
                                        ...
                                        ->exists();
                                @endphp
                                <button type="button" onclick="document.getElementById('approve-version-preview-modal-{{ $version->id }}').showModal()" class="btn btn-success btn-sm rounded-xl text-white font-semibold gap-1.5 px-4 shadow-xs hover:shadow-md transition-all">
                                ...
                                <button type="button" onclick="document.getElementById('reject-version-preview-modal-{{ $version->id }}').showModal()" class="btn btn-error btn-sm rounded-xl text-white font-semibold gap-1 px-3 shadow-xs hover:shadow-md transition-all">
                                ...
                                @include('approvals._approve_modal', [
                                    'document' => $document,
                                    'version' => $version,
                                    'modalId' => 'approve-version-preview-modal-' . $version->id,
                                    'hasPendingSignature' => $hasPendingSignature,
                                ])

                                {{-- Reject Version Modal --}}
                                <dialog id="reject-version-preview-modal-{{ $version->id }}" class="modal modal-bottom sm:modal-middle text-left whitespace-normal backdrop-blur-xs">
                                    <div class="modal-box p-0 overflow-hidden rounded-2xl sm:rounded-3xl border border-base-content/10 shadow-2xl bg-base-100 max-w-lg">
                                        <form method="POST" action="{{ route('approvals.reject', [$document, $version]) }}" data-prevent-double-submit="true" onsubmit="const btn = this.querySelector('button[type=submit]'); if(btn){ btn.disabled = true; btn.classList.add('opacity-75', 'cursor-not-allowed'); }">
                                            @csrf
                                            <div class="p-6 pb-4">
                                                <div class="flex items-start justify-between gap-4">
                                                    <div class="flex items-center gap-3.5">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p class="text-xs text-base-content/60 mt-0.5">{{ __('Tolak pengajuan versi v:version dokumen ini', ['version' => $version->version_number]) }}</p>
                                                        </div>
                                                    </div>
                                                    <button type="button" onclick="document.getElementById('reject-version-preview-modal-{{ $version->id }}').close()" class="btn btn-ghost btn-sm btn-circle text-base-content/50 hover:text-base-content hover:bg-base-200">
                                                        ✕
                                                    </button>
                                                </div>
                                            ...
                                            <div class="bg-base-200/40 px-6 py-4 border-t border-base-200 flex items-center justify-end gap-2.5">
                                                <button type="button" onclick="document.getElementById('reject-version-preview-modal-{{ $version->id }}').close()" class="btn btn-ghost btn-sm sm:btn-md rounded-xl font-medium text-base-content/70 hover:text-base-content px-4">
                                                    {{ __('Batal') }}
                                                </button>
