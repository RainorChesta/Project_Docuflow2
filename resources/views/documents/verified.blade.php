<x-guest-layout :title="__('Verifikasi Dokumen')" :heading="__('Status Dokumen')" :description="__('Hasil pemindaian QR Code Dokumen')" size="sm">
    <div class="text-center">
        @if($document->is_expired)
            ...
            <h2 class="text-base font-semibold text-error mb-0.5">{{ __('Dokumen Kedaluwarsa') }}</h2>
        ...
        @elseif($document->currentVersion)
            ...
            <h2 class="text-base font-semibold text-success mb-0.5">{{ __('Dokumen Valid & Terverifikasi') }}</h2>
            <p class="text-xs text-base-content/70 mb-5">{{ __('Dokumen terdaftar resmi dalam sistem :app.', ['app' => config('app.name', 'DokuFlow')]) }}</p>
        @else
        ...
        <div class="bg-base-200/50 rounded-xl p-3 text-left border border-base-300">
            <dl class="space-y-2 text-[13px]">
                <div>
                    <dt class="text-[10px] font-semibold text-base-content/50 uppercase">{{ __('Nomor Dokumen') }}</dt>
                    <dd class="font-medium">{{ $document->document_number ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-[10px] font-semibold text-base-content/50 uppercase">{{ __('Judul Dokumen') }}</dt>
                ...
                <div>
                    <dt class="text-[10px] font-semibold text-base-content/50 uppercase">{{ __('Versi Aktif') }}</dt>
                    <dd class="font-medium">{{ __('Versi :version (:date)', ['version' => $document->currentVersion->version_number, 'date' => $document->currentVersion->created_at->format('d M Y')]) }}</dd>
                </div>
                @endif
            </dl>
        </div>
